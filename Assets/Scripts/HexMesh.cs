...
public class HexMesh : MonoBehaviour
{
    ...
    void TriangulateCell(HexDirection direction, HexCell cell)
    {
        ...
        HexCell prev = cell.GetNeighbor(direction.Previous()) ?? cell;
        HexCell neighbor = cell.GetNeighbor(direction) ?? cell;
        HexCell next = cell.GetNeighbor(direction.Next()) ?? cell;
        ...
        AddTriangle(v1, cetner + HexMetrics.GetFirstCorner(direction), v3);
        AddTriangleColor(
            cell.color,
            (cell.color + prev.color + neighbor.color) / 3f,
            bridgeColor
        );

        AddTriangle(v2, v4, cetner + HexMetrics.GetSecondCorner(direction));
        AddTriangleColor(
            cell.color,
            bridgeColor,
